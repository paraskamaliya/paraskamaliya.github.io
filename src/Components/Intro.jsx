...
const Intro = () => {
    ...
    const downloadResume = () => {
        const resumeUrl = './ParasKamaliya_Resume.pdf';
        ...
        <Box m={"auto"} w={"90%"} >
            <Stack display={"flex"} m={"auto"} justifyContent={"center"} flexDirection={["column", "column", "row", "row", "row"]} alignItems={"center"} w={"100%"} borderBottom={"1px"} borderBottomColor={theme == "dark" ? "white" : "black"}>
                <Box textAlign={"left"} padding={"1rem"} fontSize={"2rem"} m={"auto"} w={["100%", "85%", "70%", "50%", "50%"]}>
                    ...
                    <a href={"./ParasKamaliya_Resume.pdf"} id="resume-link-2" download>
                        <Button rightIcon={<ExternalLinkIcon />} bg={theme === "dark" ? "white" : "black"} onClick={downloadResume} color={theme === "dark" ? "black" : "white"} _hover={{ color: "none" }} id="resume-button-2">Resume</Button>
                    </a>
                </Box>
